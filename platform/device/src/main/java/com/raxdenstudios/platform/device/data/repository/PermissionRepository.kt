...
interface PermissionRepository {
    ...
    suspend fun requestPermission(permission: Permission): Answer<com.raxdenstudios.platform.device.domain.model.PermissionResult, Failure>
}

internal class PermissionRepositoryImpl(
    private val permissionsManager: PermissionsManager,
) : com.raxdenstudios.platform.device.data.repository.PermissionRepository {
    ...
    override suspend fun requestPermission(permission: Permission): Answer<com.raxdenstudios.platform.device.domain.model.PermissionResult, Failure> =
        suspendCancellableCoroutine { continuation ->
            permissionsManager.requestPermission(
                permissions = arrayOf(permission),
                callbacks = PermissionsManager.Callbacks(
                    onGranted = {
                        val answer = Answer.Success(com.raxdenstudios.platform.device.domain.model.PermissionResult.Granted(permission))
                        continuation.resume(answer)
                    },
                    onRationale = {
                        val answer = Answer.Success(com.raxdenstudios.platform.device.domain.model.PermissionResult.Rationale(permission))
                        continuation.resume(answer)
                    },
                    onDenied = {
                        val answer = Answer.Success(com.raxdenstudios.platform.device.domain.model.PermissionResult.Denied(permission))
                        continuation.resume(answer)
                    },
                )
            )
